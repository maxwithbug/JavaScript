    ...
    constructor(Name ){
        ...
        ShowCourse = ()=>{
            if(this.Coursess.length==0){
                console.log(`${this.Name}'s enrolled coursse is : ${this.Course} `);
...
class Admission extends Student {
    constructor(name){
        ...
        /*course admission function */
            this.AssignCourse = (StudentName,StudentCourse)=>{
                if(typeof(StudentName.Name)==='string'){
                    this.Coursess.push(StudentCourse)
                    console.log(` ${StudentName.Name} has been enrolled in ${StudentCourse}`);
                }else{
                    console.log(`Wrong input , please Check`);
                }
            }
